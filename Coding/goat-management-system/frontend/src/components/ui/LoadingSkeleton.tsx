...
export function CardSkeleton({ className = '' }: { className?: string }) {
  return (
    <div className={`goat-card-angular animate-pulse space-y-3 ${className}`}>
      <div className="h-2.5 bg-white/[0.05] w-20 rounded-none" />
      <div className="h-8 bg-white/[0.05] w-14 rounded-none" />
    </div>
  )
}
    ...
    <tr className="border-b border-white/5 animate-pulse">
      {Array.from({ length: cols }).map((_, i) => (
        <td key={i} className="py-3 pr-4">
          <div className="h-3 bg-white/[0.05] rounded-none" style={{ width: `${60 + Math.random() * 30}%` }} />
        </td>
      ))}
    </tr>
  )
